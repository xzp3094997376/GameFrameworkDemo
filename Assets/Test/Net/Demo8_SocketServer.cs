...
public class Demo8_SocketServer
{
    ...
    //定义一个集合，存储客户端信息
    static Dictionary<string, Socket> clientConnectionItems;
    ...
    static void recv(object socketclientpara)
    {
        ...
        while (isOpen)
        {
            ...
            try
            {
                ...
                //UnityEngine.Debug.Log($"{length}长度为0");
                if (length == 0)
                {
                    UnityEngine.Debug.Log("长度为0");
                    break;
                }
                //string str = Encoding.UTF8.GetString(arrServerRecMsg);
                //Log.Info(str + " 长度为0");
